...
class MaxBotApplication:
        ...
        if state:
            ...
            if state.state == "awaiting_name":
                ...
                await settings_service.start_change_gender(
                    ...
                    resume_data={k: v for k, v in state_data.items() if k != "initial_prompt"},
                )
                return
    ...
    async def handle_callback(self, callback: IncomingCallback) -> None:
        ...
        if data == "disclaimer_accepted":
            ...
            state_data = snapshot.data if snapshot else {}
            await self.states.clear(user_id)
            self.spawn_user_task(user_id, common.resume_pending_ai_turn(self.client, chat_id, user_id, state_data, self.states))
            return
        if data.startswith("gender_"):
            ...
            if state_data.get("is_onboarding"):
                # Onboarding complete — show main menu and continue with pending turn
                await self.client.send_message(
                    chat_id=chat_id,
        ...
        if await common.is_admin(user_id):
            ...
            if data.startswith("admin_ai_reasoning_"):
                await admin_ai_service.save_model_reasoning(self.client, chat_id, data.replace("admin_ai_reasoning_", "", 1))
                return
            ...
            if data.startswith("admin_ai_log_"):
                ...
                try:
                    ...
                    return
                if not admin_ai_logs_service.validate_filters(period, request_type, status) or page < 0:
            ...
            if data.startswith("export_ai_logs_"):
                ...
                try:
                    ...
                    log.warning("Malformed export_ai_logs callback payload: %s", data)
                    return
                if not admin_ai_logs_service.validate_filters(period, request_type, status):
            ...
            if data.startswith("admin_topic_toggle_list_"):
                ...
                return
            if data.startswith("admin_topic_toggle_auto_start_"):
                await admin_topics_service.toggle_auto_start(self.client, chat_id, int(data.rsplit("_", 1)[1]))
                return
