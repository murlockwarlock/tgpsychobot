...
async def list_plans(client: MaxApiClient, chat_id: int) -> None:
    ...
    text = "📦 <b>Тарифы</b>\n\nВыберите тариф для редактирования." if plans else "📦 <b>Тарифы</b>\n\nТарифов пока нет."
...
async def show_plan_editor(client: MaxApiClient, chat_id: int, plan_id: int) -> None:
    ...
    text = (
        ...
        f"<b>ID:</b> {plan.id}\n"
        f"<b>Цена:</b> {plan.price:.2f} руб.\n"
        f"<b>Длительность:</b> {_format_duration(plan)}\n"
        f"<b>Активен:</b> {'да' if plan.is_active else 'нет'}\n"
        f"<b>Только для админов:</b> {'да' if getattr(plan, 'admin_only', False) else 'нет'}\n"
        f"<b>Пробный тариф:</b> {'да' if plan.is_trial else 'нет'}\n"
        f"<b>Автопродление:</b> {'да' if getattr(plan, 'allow_auto_renewal', True) else 'нет'}\n"
        f"<b>Апгрейд после триала:</b> {html.escape(upgrade_name)}\n"
        f"<b>Кулдаун триала:</b> {plan.trial_cooldown_days} дн.\n\n"
        f"<b>Описание:</b>\n<pre><code>{html.escape(plan.description or 'Не задано')}</code></pre>"
    )
...
async def show_plan_upgrade_menu(client: MaxApiClient, chat_id: int, plan_id: int) -> None:
    ...
    text = (
        f"Выберите целевой тариф для «{html.escape(plan.name)}».\n\n"
...
async def list_promocodes(client: MaxApiClient, chat_id: int) -> None:
    ...
    text = "🎁 <b>Промокоды</b>\n\nВыберите промокод для редактирования." if promos else "🎁 <b>Промокоды</b>\n\nПромокодов пока нет."
...
async def show_promo_editor(client: MaxApiClient, chat_id: int, promo_id: int) -> None:
    ...
    text = (
        ...
        f"<b>Активен:</b> {'да' if promo.is_active else 'нет'}\n"
...
async def show_promo_assign_menu(client: MaxApiClient, chat_id: int, promo_id: int) -> None:
    ...
    text = (
        f"Привязка тарифов к промокоду «{html.escape(promo.code)}».\n\n"
