...
`admin_content_preferences` и старые per-admin locale helpers оставлены для безопасного rollback, но больше не используются обычным runtime flow. Existing EN/PT `BotTranslation` rows читаются без копирования и удаления. При добавлении режима в уже существующую базу `init_db()` устанавливает его в `ВЫКЛ` независимо от прежних enabled locales, user selector, translation rows или revision. После этого значение меняется только явным переключателем админа для конкретного bot/database.
